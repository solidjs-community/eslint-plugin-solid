...
| `no-restated-default-options`        | error | Certain (after fix) | **Fixed in this audit.** Previously matched `<For>`/`<Show>`/`<Match>` by element name alone, so a same-named component from another library would have its (differently-defaulted) prop deleted by the autofix. Now resolves through the import map: only Solid imports match, aliases included. |
